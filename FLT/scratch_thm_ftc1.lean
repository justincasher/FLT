...
theorem ftc1 {f : ℝ → ℝ} {a b : ℝ}
  ...
  have hx_mem : x ∈ uIcc a b := Ioo_subset_Icc_self hx
  ...
  have hmeas : StronglyMeasurableAtFilter f (𝓝 x) volume :=
    (hf.mono Ioo_subset_Icc_self).stronglyMeasurableAtFilter isOpen_Ioo x hx
  have hcont_x : ContinuousAt f x :=
    (hf x hx_mem).continuousAt (Icc_mem_nhds hx.1 hx.2)
